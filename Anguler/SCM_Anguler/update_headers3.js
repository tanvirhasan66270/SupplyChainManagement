...
const dashboards = [
    ...
    ["sales-dashboard/sales-dashboard.component.html", "Sales Officer", "Leads funnel and accounts pipeline metrics."]
];
...
const template = `  <div class="dashboard-header bg-white p-4 rounded-4 shadow-sm border mb-4 d-flex justify-content-between align-items-center">
    <div>
      ...
      <p class="text-muted fs-8 mb-0 d-flex align-items-center gap-2">
        <span class="text-dark fw-bold">{role}</span> &bull; {subtitle}
...
for (let [relPath, role, subtitle] of dashboards) {
    ...
    // Replace the exact dashboard-header div that is already there from the previous run
    content = content.replace(/<div class="dashboard-header bg-white p-4 rounded-4 shadow-sm border mb-4 d-flex justify-content-between align-items-center">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/, headerHtml);
    
    fs.writeFileSync(p, content, 'utf8');
}
console.log("Third attempt complete");
