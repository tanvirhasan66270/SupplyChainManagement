...
class SalesOfficerProfileScreen extends ConsumerStatefulWidget {
  const SalesOfficerProfileScreen({super.key});

  @override
  ConsumerState<SalesOfficerProfileScreen> createState() => _SalesOfficerProfileScreenState();
}

class _SalesOfficerProfileScreenState extends ConsumerState<SalesOfficerProfileScreen> {
  ...
  String _resolveImageUrl(String? imgPath) {
    ...
    return '${ApiConstants.imgUrl}sales_officer/$trimmed';
  }
  ...
  void _populateControllers(SalesOfficerResponseDTO officer) {
    if (_lastOfficerId == officer.id && !_isEditing) return;
    _lastOfficerId = officer.id;
    _nameController.text = officer.name;
    _emailController.text = officer.email;
    _phoneController.text = officer.phone;
    _addressController.text = officer.address;
    _genderController.text = officer.gender;
    _dobController.text = officer.dob;
    _nidController.text = officer.nidNumber;
    _designationController.text = officer.designation;
    _joiningDateController.text = officer.joiningDate;
    _languageController.text = officer.language;
  }
  ...
  Future<void> _selectDate(TextEditingController controller) async {
    ...
    final initialDate = DateTime.tryParse(controller.text) ?? DateTime(1995, 1, 1);
  ...
  Future<void> _updateProfile(SalesOfficerResponseDTO officer) async {
    setState(() => _isUpdating = true);
    final currentContext = context;

    try {
      final repo = ref.read(salesOfficerRepositoryProvider);
      final request = SalesOfficerRequestDTO(
        id: officer.id,
        name: _nameController.text.trim(),
        email: _emailController.text.trim(),
        phone: _phoneController.text.trim(),
        address: _addressController.text.trim(),
        gender: _genderController.text.trim().isEmpty ? officer.gender : _genderController.text.trim(),
      ...
      await repo.update(officer.id, request, imageFile);
      ...
      if (mounted && currentContext.mounted) {
        ScaffoldMessenger.of(currentContext).showSnackBar(
          const SnackBar(
            content: Text('Sales Officer Profile Updated Successfully!'),
  ...
  Widget build(BuildContext context) {
    final officerAsync = ref.watch(currentSalesOfficerProvider);

    return Scaffold(
      backgroundColor: AppTheme.light,
      appBar: AppBar(
        title: const Text('Sales Officer Profile', style: TextStyle(color: AppTheme.dark, fontWeight: FontWeight.bold, fontSize: 18)),
      ...
      body: officerAsync.when(
        data: (officer) {
          if (officer == null) {
            return const Center(child: Text('Sales Officer profile not found.'));
          }
          _populateControllers(officer);
          return _buildProfileBody(officer);
        },
  ...
  Widget _buildProfileBody(SalesOfficerResponseDTO officer) {
    // Dynamic Completion Calculation
    int completedFields = 0;
    final totalFields = 10;
    ...
    if (_joiningDateController.text.isNotEmpty) completedFields++;
    if (officer.image.isNotEmpty || _selectedImage != null) completedFields++;
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              const Text('STAFF REGISTRY DETAILS', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppTheme.dark)),
              if (!_isEditing)
                InkWell(
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Text('Last Updated: ${officer.updatedAt}', style: const TextStyle(color: AppTheme.grey, fontSize: 11)),
              ],
            ),
          ),
          const SizedBox(height: 24),
  ...
  Widget _buildHeader(SalesOfficerResponseDTO officer) {
    ImageProvider? imageProvider;
    if (_selectedImage != null) {
      imageProvider = FileImage(File(_selectedImage!.path));
    } else {
      final resolvedUrl = _resolveImageUrl(officer.image);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildCompletionStep('Staff Identity Node', _nameController.text.isNotEmpty && _designationController.text.isNotEmpty),
  ...
  Widget _buildSettingsForm(SalesOfficerResponseDTO officer) {
    return Container(
      decoration: BoxDecoration(
        color: AppTheme.white,
        borderRadius: BorderRadius.circular(12),
  ...
  Widget _buildLocationSection(SalesOfficerResponseDTO officer) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _locationMeta('Police Station', officer.policeStationName),
          const Divider(color: AppTheme.borderGrey, height: 24),
          const Text('Detailed Registry Address HQ', style: TextStyle(fontSize: 10, color: AppTheme.grey, fontWeight: FontWeight.bold)),
          ...
          if (_isEditing)
            ...
            Text(
              _addressController.text.isEmpty ? officer.address : _addressController.text,
              style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: AppTheme.dark),
            ),
        ],
      ),
